...
#include <netinet/tcp.h> // Necessário para struct tcp_info
...
// --- NOVO: Função para exibir estatísticas de rede em tempo real (CORRIGIDA) ---
void print_connection_stats(int socket_fd, char* client_ip, int allocated_rate) {
    struct tcp_info info;
    socklen_t len = sizeof(info);
    
    // Pergunta ao Kernel as estatísticas dessa conexão TCP específica
    if (getsockopt(socket_fd, IPPROTO_TCP, TCP_INFO, &info, &len) == 0) {
        // O RTT vem em microssegundos, convertemos para milissegundos
        double rtt_ms = (double)info.tcpi_rtt / 1000.0;
        
        // Visual "Dashboard" colorido
        printf("\n\033[1;36m=============================================\033[0m\n");
        printf(" \033[1;33m[MONITORAMENTO EM TEMPO REAL - %s]\033[0m\n", client_ip);
        printf("\033[1;36m=============================================\033[0m\n");
        printf(" \033[1;32m->\033[0m QoS Alocado (Limite): \033[1;37m%d kbps\033[0m\n", allocated_rate);
        printf(" \033[1;32m->\033[0m RTT Estimado (Kernel): \033[1;37m%.2f ms\033[0m\n", rtt_ms);
        printf(" \033[1;32m->\033[0m Variação do RTT (Jitter): \033[1;37m%.2f ms\033[0m\n", (double)info.tcpi_rttvar / 1000.0);
        // Removemos as linhas de contagem de pacotes para garantir compatibilidade
        printf("\033[1;36m=============================================\033[0m\n\n");
    } else {
        // Se falhar (ex: não for Linux), apenas ignora o painel silenciosamente ou avisa
        // perror("Falha ao obter estatísticas TCP"); 
    }
}
